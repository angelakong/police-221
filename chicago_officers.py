...
features_temp = list()
# Check if complaint has been sustained
for allegation in allegations:
    associated_officer = officers[allegation['officer_associated']]
    if allegation['result'] == 'Sustained':
        associated_officer['officer_result'] = 1
    elif allegation['result'] == 'Unsustained' and associated_officer['officer_result'] != 1:
        associated_officer['officer_result'] = 0
    elif allegation['result'] != 'Unsustained' and associated_officer['officer_result'] != 1:
        associated_officer['officer_result'] = -1

for allegation in allegations:
    associated_officer = officers[allegation['officer_associated']]
    if associated_officer['officer_result'] == -1:
        continue
    else:
        results.append(associated_officer['officer_result'])
    feature_vector = collections.defaultdict(int)
    feature_vector['allegation_count'] = associated_officer['allegation_count']
    feature_vector[associated_officer['officer_race']] = 1
    feature_vector[associated_officer['officer_rank']] = 1
    feature_vector[associated_officer['officer_gender']] = 1
...
# plt.title('ROC Curve based on Officer')
